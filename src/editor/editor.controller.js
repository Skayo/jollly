...
module.exports = function($rootScope, $scope, AlertManager, $uibModal) {
	...
	$scope.filter = function(action) {
		if (action === 'missings') {
			...
			var missingsKey = Object.keys($scope.metadata).filter(function(
				key
			) {
			...
			Object.keys($scope.metadata)
				...
				.forEach(function(line, lineIndex) {
					if ($scope.metadata[line].hasOwnProperty('missing')) {
						if (
							specific &&
							$scope.metadata[line].missing !== specific
						) {
							return false
						}

						var inputIndex =
							$scope.metadata[line].missing === 'lhs' ? 0 : 1
						var selector =
	...
	var constructLangObject = function() {
		...
		;['lhs', 'rhs'].forEach(function(lang) {
			...
			if (Object.keys(obj).length > 1) {
				$scope['lang' + capitalize(lang)] = $scope[lang + 'Filename']
				tmp = {
					language: lang,
					translations: flattenObject($scope[lang])
				}
			} else {
	...
	$scope.$on('filereaded', function(event, arg, filename) {
		...
		// Set filename
		$scope[arg.state + 'Filename'] = arg.filename.replace(/(.json)$/g, '')
	...
	var constructObj = function(array, lang) {
		...
		delete $scope.tmpObj
		return lang === 'lhs' ? tmp[$scope.langLhs] : tmp[$scope.langRhs]
	}

	var unflattenObj = function(array) {
		var result = {}
		for (var i in array) {
			var keys = i.split('/')
	...
	$scope.export = function(data, lang) {
		...
		if ($scope.ratioMissing !== 100) {
			AlertManager.add({
				type: 'danger',
				msg:
					'Nicht alle Felder wurden ausgefüllt, bitte füllen Sie alle Felder aus, bevor Sie exportieren.'
			})
		}
		...
		modalInstance.result.then(function(modalResult) {
			...
			modalResult.filename = /.json/.test(modalResult.filename)
				? modalResult.filename
				: modalResult.filename + '.json'
			...
			var unflattened = unflattenObj(constructed)
			console.log('Unflattened', unflattened)
			var jsonToExport = JSON.stringify(unflattened, null, 2)
			console.log('JsonToExport', jsonToExport)
			var blob = new window.Blob([jsonToExport], {
